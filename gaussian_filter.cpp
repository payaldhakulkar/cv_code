...
{
  int array_kernel_x[25] = {2, 4, 5, 4, 2, 4, 9, 12, 9, 4, 5, 12, 15, 12, 5, 4, 9, 12, 9, 4, 2, 4, 5, 4, 2};
  //int array_kernel_x[9] = {4, 7, 4, 7, 15, 7, 4, 7, 4};
  //int array_kernel_x[9] = {1, 2, 1,  2, 4, 2, 1, 2, 1};
  int k, l, sum;
  sum=0;
  for(k=0 ; k<5 ; k++)
     {
        for (l=0; l<5; l++)
            {
              sum = sum + (temp[(i+k) * width + (j+l)] * array_kernel_x[k * 5 + l]);
            }
     }
   sum = sum/159;
  return sum;
